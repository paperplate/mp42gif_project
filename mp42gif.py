...
def writeimgdata(r,g,b,tbl):
	...
	for e in tbl:
		...
		codes[e] = [n]
		n+=1
	# table size is 6 so special codes go in index 128, 129
	codes['CC'] = [256]
	codes['EOI'] = [257]
	original = codes.copy()
	...
	indexbuffer+=codes[(parsecol(r[0][0],'r'),
				parsecol(g[0][0],'g'),
				parsecol(b[0][0],'b'))]
	#index stream
...
def makecolourtbl(r,g,b):
	...
	for i in range(8):
		for j in range(8):
			for k in range(4):
				tbl.add((parsecol(i*32, 'r'),
					parsecol(j*32, 'g'),
					parsecol(k*64, 'b')))
	vid = bytearray()
	for col in tbl:
		for ch in col:
			vid.extend(struct.pack('<B',ch))
